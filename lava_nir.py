...
def read_node(network: typing.Any) -> nir.NIRNode:
    ...
    for layer_idx_int in sorted(list(map(int, network.keys()))):
...
def read(filename: typing.Union[str, pathlib.Path]) -> nir.NIRGraph:
    ...
    with h5py.File(filename, "r") as f:
        return read_node(f["layer"])

if __name__ == '__main__':
    logging_level = logging.DEBUG
    logging.basicConfig(level=logging_level, format='[lava_nir]:[%(levelname)s] %(message)s')
    net_filename = 'lava-dl/tutorials/lava/lib/dl/netx/oxford/Trained/network.net'
    net_filename = 'lava-dl/tutorials/lava/lib/dl/netx/pilotnet_snn/network.net'
    logging.debug('using %s', net_filename)
    net = netx.hdf5.Network(net_config=net_filename)
    nir_graph = read(net_filename)
